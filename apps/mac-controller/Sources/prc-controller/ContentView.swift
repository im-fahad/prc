    ...
    private var title: some View {
        HStack(spacing: 6) {
            if let id = model.selectedHostId, let host = model.store.host(id) {
                ...
                Text(host.name).font(.system(size: 12, weight: .medium)).foregroundStyle(Theme.text)
                Text(host.fingerprint).font(.system(size: 10, design: .monospaced)).foregroundStyle(Theme.textFaint)
            } else {
                Text("No host selected").font(.system(size: 12)).foregroundStyle(Theme.textDim)
            }
            Text(model.stateSummary).font(.system(size: 11)).foregroundStyle(Theme.textDim)
        }
        .lineLimit(1)
    }
    ...
    private var connectedControls: some View {
        HStack(spacing: 6) {
            ...
            Menu {
                ...
                Label("Keys", systemImage: "command").font(.system(size: 11))
            }
            .menuStyle(.borderlessButton)
            .fixedSize()
            .foregroundStyle(Theme.textDim)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text("THIS MAC").font(.system(size: 9, weight: .semibold)).foregroundStyle(Theme.textFaint).tracking(0.6)
                Text(model.fingerprint).font(.system(size: 13, design: .monospaced)).foregroundStyle(Theme.text)
    ...
    var body: some View {
        HStack(spacing: 7) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(host.name).font(.system(size: 12)).foregroundStyle(Theme.text).lineLimit(1)
                Text(host.fingerprint).font(.system(size: 9, design: .monospaced)).foregroundStyle(Theme.textFaint)
            }
            Spacer(minLength: 0)
        }
...
struct ScreenArea: View {
    ...
    private var placeholder: some View {
        VStack(spacing: 10) {
            ...
            Text(model.placeholderTitle).font(.system(size: 13)).foregroundStyle(Theme.textDim)
            if let detail = model.placeholderDetail {
                Text(detail).font(.system(size: 11)).foregroundStyle(Theme.textFaint)
...
struct LogPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 8) {
                Text("LOG").font(.system(size: 9, weight: .semibold)).tracking(0.6).foregroundStyle(Theme.text)
                Spacer()
...
struct StatusBar: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            Text(model.fingerprint).font(.system(size: 10, design: .monospaced)).foregroundStyle(Theme.textFaint)
        }
...
struct PairingSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Pair with a host").font(.system(size: 16, weight: .semibold)).foregroundStyle(Theme.text)
            ...
            Text(model.fingerprint).font(.system(size: 20, design: .monospaced)).foregroundStyle(Theme.text)
            ...
            if !model.pairingStatus.isEmpty {
                Text(model.pairingStatus).font(.system(size: 11)).foregroundStyle(Theme.warn)
            }
            HStack {
                Spacer()
